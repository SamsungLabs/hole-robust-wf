...
Usage:
    process_hole_robust.py [options] <yaml-config> <checkpoint> <image-dir> <mask-dir> <output-dir>
    process_hole_robust.py (-h | --help )

Arguments:
   <yaml-config>                 Path to the yaml hyper-parameter file
   <checkpoint>                  Path to the checkpoint
   <image-dir>                   Path to the directory containing processed images
   <mask-dir>                    Path to the directory containing processed masks
...
from lcnn_hole_robust.datasets import WireframeDataset, collate
...
def main():
    ...
    loader = torch.utils.data.DataLoader(
        WireframeDataset(args["<image-dir>"], split="valid", valid_mask_dir=args["<mask-dir>"]),
        shuffle=False,
    ...
    for batch_idx, (image_gt, image, meta, target) in enumerate(loader):
        with torch.no_grad():
            input_dict = {
                "image_gt": recursive_to(image_gt, device),
