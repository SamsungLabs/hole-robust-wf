...
from lcnn_hole_robust.models.rgb_gan import RGBGANDiscriminatorLearner, RGBGANDiscriminatorOptimizer
...
class Trainer(object):
    def __init__(self, device, model, optimizer, train_loader, val_loader, out):
        ...
        self.model = model
        if C.io.is_rgb_gan:
            self.model_rgb_gan = RGBGANDiscriminatorLearner()
            if C.io.resume_from:
                checkpoint_adv = torch.load(osp.join(C.io.resume_from, "discriminator_latest.pth"))
                self.model_rgb_gan.load_state_dict(checkpoint_adv["discriminator"])
            self.model_rgb_gan = self.model_rgb_gan.to(device)
            self.optim_rgb_gan = RGBGANDiscriminatorOptimizer(model=self.model_rgb_gan, lr=0.1*C.optim.lr).optimizer
        else:
            self.model_rgb_gan = None
            self.optim_rgb_gan = None
    ...
    def _loss(self, result, list_exclude=["dis"]):
        ...
        for i in range(self.num_stacks):
            for j, name in enumerate(self.loss_labels):
                ...
                if name in list_exclude:
                    continue
                total_loss += loss
        return total_loss

    def _total_loss_lcnn(self, result, keys=["jmap","joff","lmap","lpos","lneg"]):
        total_loss_lcnn = 0
        key_list = list(result["losses"][0].keys())
        for key in keys:
            if key in key_list:
                total_loss_lcnn += result["losses"][0][key].mean().item()
        return total_loss_lcnn

    def validate(self):
        tprint("Running validation...", " " * 75)
        training = self.model.training
        self.model.eval()
        if self.model_rgb_gan is not None:
            self.model_rgb_gan.eval()
        ...
        self.metrics[...] = 0
        with torch.no_grad():
            for batch_idx, (image_gt, image, meta, target) in enumerate(self.val_loader):
                input_dict = {
                    "image_gt": recursive_to(image_gt, self.device),
                ...
                if self.model_rgb_gan is not None:
                    result_adv = self.model_rgb_gan(input_dict, result, is_d=True, is_g=True)
                    for key in ["dis", "gen", "per", "sty"]:
                        result["losses"][0][key] = result_adv["losses_%s" % key]

                total_loss += self._loss(result)
                total_loss_lcnn += self._total_loss_lcnn(result)
        ...
        if self.model_rgb_gan is not None:
            torch.save({"discriminator": self.model_rgb_gan.state_dict()}, osp.join(self.out, "discriminator_latest.pth"))
            shutil.copy(
                osp.join(self.out, "discriminator_latest.pth"),
                osp.join(self.out, "discriminator_{:0>8}.pth".format(self.iteration)),
            )
        self._write_metrics(len(self.val_loader), total_loss, "validation", True)
        self.mean_loss = total_loss_lcnn / len(self.val_loader)
        self.writer.add_scalar("validation/total_loss_lcnn", self.mean_loss, self.iteration)
        ...
        shutil.copy(
            osp.join(self.out, "checkpoint_latest.pth"),
            osp.join(self.out, "checkpoint_{:0>8}.pth".format(self.iteration)),
        )
    ...
    def train_epoch(self):
        self.model.train()
        if self.model_rgb_gan is not None:
            self.model_rgb_gan.train()
        self.train_loader.dataset.epoch = self.epoch
        torch.manual_seed(self.epoch)

        time = timer()
        for batch_idx, (image_gt, image, meta, target) in enumerate(self.train_loader):
            ...
            if M.gan_optimization_mode == 1:
                # Get adversarial losses of generator and discriminator
                if self.model_rgb_gan is not None:
                    result_adv = self.model_rgb_gan(input_dict, result, is_d=True, is_g=True)
                    for key in ["dis", "gen", "per", "sty"]:
                        result["losses"][0][key] = result_adv["losses_%s" % key]

                # Update model parameters of generator
                ...
                if self.model_rgb_gan is not None:
                    self.optim_rgb_gan.zero_grad()
                    result_adv["losses_dis"].backward()
                    self.optim_rgb_gan.step()

            elif M.gan_optimization_mode == 2:
                # Release model parameters in discriminator
                if self.model_rgb_gan is not None:
                    for p in self.model_rgb_gan.parameters():
                        p.requires_grad = True

                # Get adversarial loss of discriminator
                if self.model_rgb_gan is not None:
                    result_adv = self.model_rgb_gan(input_dict, result, is_d=True, is_g=False)
                    for key in ["dis"]:
                        result["losses"][0][key] = result_adv["losses_%s" % key]
                ...
                # Freeze model parameters in discriminator
                if self.model_rgb_gan is not None:
                    for p in self.model_rgb_gan.parameters():
                        p.requires_grad = False

                # Get adversarial loss of generator
                if self.model_rgb_gan is not None:
                    result_adv = self.model_rgb_gan(input_dict, result, is_d=False, is_g=True)
                    for key in ["gen", "per", "sty"]:
                        result["losses"][0][key] = result_adv["losses_%s" % key]
                ...
                raise ValueError("undefined M.gan_training_mode")
            ...
            self.writer.add_scalar("training/total_loss_lcnn", self._total_loss_lcnn(result), self.iteration)
